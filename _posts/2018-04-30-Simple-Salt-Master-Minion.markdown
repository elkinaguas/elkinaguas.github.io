...
sudo salt '*' test.ping
```
With this command the server is telling to all the minions to execute `test.ping` in parallel and return the result. The result of this action should be something like this:
```
<device_name>:
    True
```
